...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    def _render_eds_tab(company: str, tab_idx: int):
                ...
                if not df_eds_c.empty:
                    ...
                    if "comuna" in df_eds_c.columns:
                        _eds_ciudad_map = dict(zip(
                            df_eds_c["eds_occim"].astype(str),
                            df_eds_c["comuna"].fillna("—").astype(str).str.title(),
                        ))
        ...
        if "Comuna" in _df_display.columns:
            _df_display["Comuna"] = _df_display["Comuna"].astype(str).apply(
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────
    if _util_sub_tab == "🗓️ Programación STO":
        from openpyxl import load_workbook as _lwb
        import calendar as _cal_sto
        from datetime import date as _date_sto

        st.title("🗓️ Programación STO — Cronograma diario de técnicos")
        st.caption(
    ...
    # ── Sub-tab: Cronograma de Turnos STO ─────────────────────────────────────
    if _util_sub_tab == "📅 Cronograma de Turnos":
...
| **TOTAL excluidos** | **{_n_excl:,}** | | = Solo Trabajos Especiales |
    ...
    if _main_tab == _MT_PREC:
        ...
        # ── Botón descarga Excel resumen mensual ─────────────────────────
        if len(_meses_sel_raw) == 1 and (equipo_kpi != "Todos" or tec_kpi_sel != "Todos"):
            _dl_mes = _meses_sel_raw[0]
                ...
                if not ot_hist.empty and "_periodo" in ot_hist.columns:
                    ...
                    if _cumpl_k not in st.session_state:
                        ...
                        fig_cumpl.update_layout(
                            ...
                            yaxis=dict(title="Cumplimiento (%)", range=[50, 102],
                                       tickmode="linear", tick0=50, dtick=10,
                                       showgrid=True,
...
# ══════════════════════════════════════════════════════════════════════════
    ...
    if False:  # dead code preservado por si necesitamos referencia
        ...
        # 2. Precisión — SIEMPRE computar fresco (no depender de session_state
        #    porque las sub-pestañas usan st.radio y solo la activa poblará cache).
        _prec_records = []
        _prec_err = None
        try:
            ...
            if not _kpi_exp.empty:
                ...
                if not _ot_full.empty:
                    ...
                    _prec_g = _ot_full.groupby(["tecnico","equipo","mes_num"]).agg(
                        buenas=("buena","sum"), total=("buena","count")).reset_index()
                    _prec_g["tecnico"] = _prec_g["tecnico"].str.replace(r'\s+',' ',regex=True).str.strip()
                    _prec_records = _prec_g.to_dict(orient="records")
        except Exception as _e_prec_exp:
            _prec_err = f"{type(_e_prec_exp).__name__}: {_e_prec_exp}"

        # 3. Efectividad — reincidencias SIEMPRE fresco
        _reinc_records = []
        _reinc_err = None
        try:
        ...
        except Exception as _e_reinc_exp:
            _reinc_err = f"{type(_e_reinc_exp).__name__}: {_e_reinc_exp}"
        ...
        _export_obj = {
            ...
            "_debug": {
                "raw_wo_count": len(raw_wo) if raw_wo else 0,
                "df_wo_count": len(df_wo) if not df_wo.empty else 0,
                "prec_err": _prec_err,
                "reinc_err": _reinc_err,
                "sla_count": len(_sla_records),
                "prec_count": len(_prec_records),
                "reinc_count": len(_reinc_records),
                "pm_count": len(_pm_records),
            },
        }

        # Alerta visible en el dashboard si el export dejó datos vacíos
        _empty_sections = []
        if not _prec_records: _empty_sections.append(f"Precisión ({_prec_err or 'sin datos'})")
        if not _reinc_records: _empty_sections.append(f"Reincidencias ({_reinc_err or 'sin datos'})")
        if _empty_sections:
            st.warning(
                "⚠️ Export STO app móvil incompleto — secciones vacías: "
                + " · ".join(_empty_sections)
            )
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    #        Ejemplo: prog 04/08. Ejec 09/08 (5d tarde) → CUMPLE (frontera).
    #                 Ejec 10/08 (6d tarde) → NO cumple (excede).
    #                 Rango: 15 días antes → 5 días después (día 6 ya es exceso).
    #
    ...
    #        Exceso normal  ⇔  ejecutada  AND  6 ≤ dias_atraso < 15   (6 a 14 días tarde)
    ...
    if not _df_cumpl.empty:
        ...
        _df_cumpl["_exceso"]        = _ej & (_da >= 6) & (_da < 15)
    ...
    def _build_exceso_gauge(pct_total: float, pct_critico: float) -> "_go.Figure":
        ...
        fig = _go.Figure(_go.Indicator(
            ...
            value=pct_total,
            number={"suffix": " %", "font": {"size": 32}},
            title={"text": "<b>% Exceso (≥6 días tarde)</b><br>"
    ...
    with _gc2:
        st.plotly_chart(
            _build_gauge(_cump_sem_pct, "% Cumplimiento — Intervalo Flexible",
                         "ventana: 15 días antes → 5 días después (día 6 ya es exceso)"),
    ...
    st.markdown(
        f"""<div style="padding:14px 18px;background:rgba(148,163,184,0.08);
          ...
          <table style="width:100%;border-collapse:collapse;font-size:0.92rem;">
            ...
            <tr style="border-bottom:1px solid rgba(148,163,184,0.25);">
              <td style="padding:6px 0;font-weight:600;color:var(--text-color, #0f172a);">
                Intervalo Flexible (15 días antes → 5 días después; día 6 ya es exceso)</td>
              <td style="padding:6px 0;text-align:right;">
            ...
            <tr style="border-bottom:1px solid rgba(148,163,184,0.25);">
              <td style="padding:6px 0;font-weight:600;color:var(--text-color, #0f172a);">
                Exceso (6 a 14 d tarde) + Exceso crítico (≥15 d tarde)</td>
              <td style="padding:6px 0;text-align:right;">
